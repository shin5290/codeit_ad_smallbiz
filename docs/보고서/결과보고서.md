...
2. **RAG 기반 챗봇 시스템 엔드포인트 연결**: 의도 분석(강도 포함), 맥락 정제, 생성 파이프라인 통합
...
5. **데이터 관리**: 향후 업종 추가 시 본 21개 로직에 따라 비주얼 의존도를 체크하여 배치
...
## 7\. 결론 및 향후 계획
### 7.1 프로젝트 성과
본 프로젝트는 생성형 AI 기술을 활용하여 소상공인의 광고 콘텐츠 제작 진입 장벽을 낮추는 것을 목표로 하였으며, RAG 챗봇, 이미지 생성, 광고 카피 작성, 그리고 업종별 전략 수립의 전 과정에서 유의미한 기술적, 정량적 성과를 거두었습니다.
#### 7.1.1 기술적 성과
**(1) 리소스 효율적인 고품질 이미지 생성 파이프라인 구축**
- 노드 기반 아키텍처: 확장성과 유지보수성이 뛰어난 노드(Node) 단위 워크플로우를 설계하여 T2I, I2I, 제품 배경 합성 등 다양한 시나리오에 유연하게 대응했습니다.

- 메모리 최적화: 공유 캐시(Shared Cache) 시스템과 순차 실행 전략을 통해, 고성능 모델(Z-Image Turbo)을 GCP L4(24GB VRAM) 환경에서도 OOM 없이 안정적으로 구동했습니다. (메모리 사용량 40% 절감)

- CLIP 한계 극복: Qwen 2.5 LLM 인코더를 도입하여 77토큰 제한을 넘어 512+ 토큰의 긴 프롬프트를 처리, 소상공인의 구체적인 요구사항을 반영했습니다.

**(2) 실용적인 RAG 기반 상담 시스템 구현**
- Simple is Best 증명: 복잡한 Hybrid 검색이나 Re-ranker 대신, 최적화된 Dense Embedding(E5) 단일 모델을 사용하여 검색 정확도(Recall@5 88.5%)와 효율성을 동시에 확보했습니다.

- 임베딩 CPU 최적화: VRAM 부족 문제를 해결하기 위해 임베딩 모델을 CPU로 오프로딩하고 큐잉 시스템을 도입하여 시스템 안정성을 확보했습니다.

- Intent Router: 사용자 의도를 91.5% 정확도로 분류하여 단순 대화, 마케팅 상담, 사례 검색을 효율적으로 라우팅했습니다.

**(3) 업종별 특화 전략 및 자동화**
- 247개 업종 분류 체계: 소상공인 업종을 마케팅 관점에서 6개 등급, 21개 그룹으로 재분류하고, 각 그룹에 최적화된 프롬프트 엔지니어링을 적용했습니다.

- 맥락 인식 카피라이팅: 대화 히스토리를 반영하는 메모리 시스템을 통해, 사용자의 피드백을 반영하며 점진적으로 개선되는 광고 문구 생성 로직을 구현했습니다.
**(4) 안정적인 백엔드 및 관리 시스템**
- SSE 스트리밍: 실시간성이 중요한 생성형 AI 서비스에 맞춰 SSE 기반의 스트리밍 응답을 구현, 사용자 체감 대기 시간을 단축했습니다.

- 통합 관리자 대시보드: 사용자 관리, 생성 이력 조회, 실시간 로그 스트리밍 기능을 포함한 관리자 페이지를 구축하여 운영 효율성을 높였습니다.

#### 7.1.2 정량적 지표

| 카테고리 | 핵심 지표 | 달성 수치 | 비고 |
|---------|-----------|----------|------|
| RAG 검색	 | Recall@5	| 88.5%	| 목표(80%) 초과 달성 |
| 시스템 안정성	| 오류율 |	0%	| 200개 쿼리 테스트 기준 |
| 비용 효율	| 월 운영 비용 |	$16	| 1만 쿼리 기준, 목표 대비 68% 절감 |
| 이미지 효율	| 스토리지 절감 |	78% ↓ |	99GB → 22GB (LoRA 활용) |
| 이미지 효율	| 메모리 절감	| 40% ↓	| 40GB → 24GB (공유 캐시) |
| 품질 평가	| 이미지 프롬프트 승률	| 99.0%	| 업종 특화(B) vs 일반(A) A/B 테스트 |
| 품질 평가	| 광고 문구 승률	| 87.6%	| 업종 특화(B) vs 일반(A) A/B 테스트 |

### 7.2 핵심 인사이트
**(1) 단순함이 복잡함을 이긴다 (Simple is Best)**
- RAG 시스템 개발 과정에서 확인했듯이, 무조건적인 최신 기술(Hybrid Search, Re-ranker) 도입보다는 데이터 특성에 맞는 기본 모델(E5 Dense)의 최적화가 성능과 비용 면에서 더 우수한 결과를 낼 수 있음을 확인했습니다.

**(2) 하드웨어 제약은 소프트웨어 아키텍처로 극복 가능하다**
- 한정된 GPU 자원(24GB VRAM) 내에서 거대 모델을 구동하기 위해, 모델을 메모리에 상주시킨 채 필요한 모듈(rembg, LoRA)만 동적으로 로드/언로드하는 전략과 공유 캐시 시스템이 유효했습니다.

**(3) 도메인 특화 데이터가 품질의 핵심이다**
- 범용적인 프롬프트보다 247개 업종을 분석하여 만든 '21개 그룹별 특화 프롬프트'가 압도적인 품질 차이(승률 99%)를 만들었습니다. AI의 성능은 모델 자체보다 도메인 지식이 결합된 프롬프트 엔지니어링에 달려있음을 입증했습니다.

**(4) 현실적인 평가 기준의 중요성**
- 단일 정답이 아닌 다중 정답(Multi-Answer) 기반의 RAG 평가, 그리고 실제 마케팅 효과 관점의 비주얼/카피 평가 기준을 수립함으로써, 과적합을 피하고 실사용 환경에 가까운 성능을 확보할 수 있었습니다.

### 7.3 한계점 및 개선 과제
#### 7.3.1 현재 시스템의 제약
**(1) 응답 지연 시간 (Latency)**
- 현재 RAG 챗봇의 평균 응답 속도는 약 8.5초로, 실시간 대화형 서비스로서는 다소 느린 편입니다. 캐싱(Redis) 도입, LLM 스트리밍 최적화, 병렬 처리를 통해 목표치인 2-3초대로 단축해야 합니다.
- 이미지 생성 역시 고품질 생성을 위해 수 초가 소요되므로, 사용자 경험을 위해 진행률 표시 외에 예상 결과 미리보기 등의 UX 개선이 필요합니다.

**(2) 업종 인식률의 한계**
- 사용자 입력에 따른 업종 자동 분류 정확도가 58.9%로 낮습니다. 특히 D등급(목적형 소매) 및 S-5/6(특수 요리/주점) 그룹의 인식률이 저조하여, industries.yaml의 키워드 보강 및 NLP 기반의 분류 모델 고도화가 시급합니다.

**(3) 이미지 내 텍스트 처리**
- Diffusion 모델의 특성상 이미지 내에 정확한 텍스트를 생성하는 데 한계가 있습니다. 현재 GPT-4V와 PIL을 이용한 후처리 방식을 사용 중이나, 더욱 자연스러운 합성을 위해 전용 텍스트 렌더링 기술이나 벡터 그래픽 통합이 필요합니다.

**(4) 확장성 (Scalability)**
- 현재 단일 GPU 인스턴스에 의존하고 있어 대규모 트래픽 발생 시 병목이 예상됩니다. 로드 밸런싱 및 분산 처리 아키텍처 도입이 필요합니다.

#### 7.3.2 사용자 피드백 부재
- **실사용자 테스트 부족**: 본 프로젝트의 성능 평가는 주로 합성 데이터(Synthetic Queries)와 팀 내부 테스트에 의존했습니다. 실제 소상공인들의 질의 패턴이나 생성된 광고의 실제 마케팅 성과(CTR 등)에 대한 데이터가 부족합니다.
- **향후 계획**: MVP 배포를 통해 실제 사용자 로그를 수집하고, 이를 바탕으로 RAG 데이터셋을 보강(Real-world Feedback)하며, 광고 성과 데이터를 피드백 루프로 활용하여 프롬프트를 지속적으로 최적화하는 과정이 필수적입니다.