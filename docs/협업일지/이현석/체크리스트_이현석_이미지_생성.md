...
### 🗓 1주차 (12/29 ~ 1/4): 환경 설정 및 SDXL 기본 연동

#### Day 1-2 (12/29-12/30): GPU 환경 설정 (GCP VM)

**Python 환경 구성**
- [V] GCP VM 접속 확인
- [V] GPU 확인: NVIDIA L4, Driver 570.x, CUDA 12.6
  - 확인 명령어: nvidia-smi
- [V] Python 가상 환경 생성
...
#### Day 3-5 (12/31-1/2): 노드 기반 아키텍처 구현

**노드 시스템 설계**
- [V] 파일 구조 생성
  - nodes/base.py: 추상 클래스 BaseNode
...
- [V] 추상 메서드 정의
  ...
  class BaseNode(ABC):
      ...
      def get_output_keys(self) -> list:
          pass
  ```
- [V] 메타데이터 추적 (NodeMetadata)
- [V] execute() 메서드로 실행 시간 측정

**ImageGenerationWorkflow 구현**
- [V] 노드 체이닝 시스템
  ```python
...
- [V] 로컬 모델 캐싱 (models/ 폴더)
- [V] 자동 언로드 메모리 관리
- [V] Variant fallback 처리 (fp16 미지원 모델 대응)

#### Day 6-7 (1/3-1/4): Text2ImageNode 구현 및 스타일 테스트

**Text2ImageNode 구현**
- [V] SDXL 파이프라인 lazy loading
  ```python
  def _load_pipeline(self):
      ...
      if local_model_path.exists():
          ...
          # HuggingFace에서 다운로드 및 캐싱
          ...
  ```
- [V] process() 메서드로 이미지 생성
- [V] aspect_ratio 지원 (1:1, 3:4, 4:3, 16:9, 9:16)
- [V] negative_prompt 개선 (손가락 관련 포함)

**config.py 작성**
- [V] AspectRatioTemplates 클래스
- [V] GenerationConfig (steps, CFG, negative prompt)
- [V] ModelConfig (디바이스, dtype, variant)
- [V] 업종별 스타일 프리셋

**스타일별 테스트 스크립트 작성**
- [V] test_workflow.py 작성
- [V] 9가지 테스트 케이스
  - Ultra Realistic: 베이커리, 바리스타, 헤어샵
  - Semi Realistic: 꽃집, 꽃집 직원, 서점
  - Anime: 카페, 바리스타, 제빵사
- [V] 각 스타일별 다른 모델 자동 로드
- [V] test_images/ 폴더에 결과 저장

**✅ 1주차 마일스톤**: 노드 기반 아키텍처 완성 및 멀티 모델 시스템 구축

---

### 🗓 2주차 (1/5 ~ 1/11): 후처리 및 최적화

#### Day 8-10 (1/5-1/7): 후처리 함수
...
#### Day 11-12 (1/8-1/9): 텍스트 오버레이

**_add_text_overlay() 함수**
- [ ] 함수 작성
  ```python
...
- [ ] 한글 텍스트 테스트
  ...
  image_with_text = generator._add_text_overlay(image, "따뜻한 겨울, 새로운 맛")
...
#### Day 13-14 (1/10-1/11): services.py 연동
...
**services.py와 통합 테스트 (진수경님과 협업)**
- [ ] services.create_advertisement()에서 호출
- [ ] 반환값 형식 확인
- [ ] 메타데이터 DB 저장 확인

**✅ 2주차 마일스톤**: 이미지 생성 + 텍스트 오버레이 완성

---

### 🗓 3주차 (1/12 ~ 1/15): 에러 처리 및 안정화

#### Day 15-17 (1/12-1/14): 에러 처리
...
#### Day 19-21 (1/16-1/18): Flux.1 모델 지원 (선택)
...
#### Day 22-24 (1/19-1/21): ControlNet (선택)
...
#### Day 25-27 (1/23-1/25): 성능 최적화

**추론 단계 조정**
- [ ] num_inference_steps 최적화
  - [ ] 빠른 생성: 20 steps
  - [ ] 기본: 30 steps
  - [ ] 고품질: 50 steps
...
- [ ] 여러 이미지 동시 생성
  ```python
  result = self.pipeline(
      prompt=[prompt] * 4,  # 4개 동시 생성
      ...
  )
  images = result.images
  ```

#### Day 28-30 (1/26-1/27): Docker 컨테이너화

**Dockerfile 작성 (신승목님과 협업)**
- [ ] GPU 지원 Docker 이미지
- [ ] 모델 가중치 포함 여부 결정

**문서화**
- [ ] UnifiedImageGenerator 사용 가이드
  ```markdown
  ## UnifiedImageGenerator 사용법
  ...
  ### 출력
  - result["image_path"]: 임시 파일 경로
  - result["metadata"]: {"seed", "style", "model"}
  ```

**✅ 최종 완료**: 이미지 생성 모듈 완성 및 배포 준비
...
**1주차**: GPU 환경 + 노드 아키텍처 + 멀티 모델 시스템
- [V] PyTorch CUDA 12.6 설치 및 GPU 확인
- [V] 노드 기반 아키텍처 구현 (BaseNode, Workflow)
- [V] 멀티 모델 시스템 (RealVisXL, Equinox, Animagine)
- [V] 로컬 캐싱 및 자동 언로드
- [V] 스타일별 테스트 스크립트 (9개 케이스)
...
### 진수경님과의 협업
- [ ] services.create_advertisement()에서 generate() 호출
- [ ] 반환값 형식 확인
  - [ ] {"image_path": str, "metadata": dict}
- [ ] 메타데이터 DB 저장 확인

### 배현석님과의 협업
- [ ] PromptTemplateManager에서 생성한 프롬프트 사용
- [ ] TextGenerator에서 생성한 ad_copy 사용

### 신승목님과의 협업
- [ ] 생성된 이미지 파일 저장 (save_image)
- [ ] 임시 파일 정리
- [ ] Docker 이미지 빌드
...
```python
# 기본 설정
num_inference_steps=30  # 추론 단계 (20-50)
...
- SDXL 모델 로드: 약 6-7GB
- 이미지 생성 시: 약 2-3GB 추가
- 총: 약 10GB 필요 (L4 GPU: 22GB, 충분함)
- 자동 언로드로 모델 교체 시에도 안정적 운영
...
## 📝 주요 구현 사항 (2026-01-05 기준)

### ✅ 완료된 기능
1. **노드 기반 아키텍처**
   - BaseNode 추상 클래스
   - ImageGenerationWorkflow 체이닝 시스템
   - 메타데이터 자동 추적
...
2. **멀티 모델 시스템**
   ...
   - 로컬 캐싱 시스템 (models/ 폴더)
   - 자동 언로드 메모리 관리
   - Variant fallback 자동 처리

3. **Text2ImageNode**
   - SDXL 파이프라인 lazy loading
   - 5가지 aspect ratio 지원
   - Negative prompt 최적화 (손가락 포함)
   - 업종별 스타일 프리셋
...
### 🚧 진행 예정
1. 전처리 노드 구현 (배경 제거, 리사이즈)
2. UnifiedImageGenerator 통합 인터페이스
3. API 엔드포인트 연결
4. 텍스트 오버레이 기능
