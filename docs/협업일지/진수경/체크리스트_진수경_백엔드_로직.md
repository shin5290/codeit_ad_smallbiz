# 진수경님 체크리스트
## 백엔드 비즈니스 로직 (services.py, db_processor.py, schemas.py)

**담당 범위**: 
- DB 설계 및 ORM 모델 (SQLAlchemy)
- DB 접근 레이어 (process_db.py)
- 데이터 스키마 (schemas.py)
- 비즈니스 로직 연결 (services.py)

**프로젝트 기간**: 2025-12-29 ~ 2026-01-27

---

## 📋 전체 구조

```
src/backend/
├── models.py           # SQLAlchemy ORM 모델 (6개 테이블)
├── process_db.py       # DB CRUD 함수
├── schemas.py          # Pydantic 스키마
├── services.py         # 비즈니스 로직 (AI 호출)
└── routes.py           # FastAPI 엔드포인트 (이유노님)

src/generation/
├── image_generation/
│   ├── generator.py    # SDXL 이미지 생성 (이현석님), 추후 작성된 내용 확인 필요
│   └── controlnet.py   # ControlNet (이현석님), 추후 작성된 내용 확인 필요
└── text_generation/
    └── prompt_manager.py  # GPT 프롬프트 생성 (배현석님), 추후 작성된 내용 확인 필요
```

---

## 📅 1단계 (MVP): ~ 2026-01-15

### 🗓 1주차 (12/29 ~ 1/4): DB 설계 및 모델 작성

#### Day 1-2 (12/29-12/30): PostgreSQL 설정

**PostgreSQL 설치 (GCP VM)**
```bash
# SSH 접속
gcloud compute ssh ad-generator-vm

# PostgreSQL 설치
sudo apt-get update
sudo apt-get install -y postgresql postgresql-contrib

# 서비스 시작
sudo systemctl start postgresql
sudo systemctl enable postgresql

# 상태 확인
sudo systemctl status postgresql
```

**데이터베이스 생성**
```bash
# PostgreSQL 접속
sudo -u postgres psql

# DB 및 사용자 생성
CREATE DATABASE adbizdb;
CREATE USER aduser WITH PASSWORD 'your_secure_password';
GRANT ALL PRIVILEGES ON DATABASE adbizdb TO aduser;

# 종료
\q

# 연결 테스트
psql -U aduser -d adbizdb -h localhost
```

- [x] PostgreSQL 14 설치
- [x] adbizdb 데이터베이스 생성
- [x] aduser 사용자 생성 및 권한 부여
- [x] 연결 테스트 완료

**SQLAlchemy 설치**
```bash
cd ~/codeit_ad_smallbiz
source venv/bin/activate

pip install sqlalchemy psycopg2-binary bcrypt python-dotenv
```

- [x] 의존성 설치 완료

---

#### Day 3-5 (12/31-1/2): models.py 작성

**파일 생성**: `src/backend/models.py`

**기본 구조**
```python
"""
SQLAlchemy ORM 모델 정의
DB 구조 v5.0 (2025-01-05)
"""

from sqlalchemy import (
    Column, Integer, String, Text, ForeignKey,
    DateTime, Boolean, func
)
from sqlalchemy.orm import relationship, declarative_base

Base = declarative_base()
```

- [x] 파일 생성 및 기본 import

**User 모델**
```python
class User(Base):
    """
    유저 정보 (계정 단위)
    - 유저 삭제 시에도 세션/이력은 남김 (SET NULL)
    """
    __tablename__ = "user"

    user_id = Column(Integer, primary_key=True, index=True)
    login_id = Column(String(50), unique=True, nullable=False, index=True)
    login_pw = Column(String(255), nullable=False)
    name = Column(String(50), nullable=False)
    created_at = Column(DateTime, server_default=func.now())

    sessions = relationship(
        "ChatSession",
        back_populates="user",
        cascade="save-update, merge",
        passive_deletes=True,
    )
```

- [x] User 클래스 작성
- [x] 필드 정의 (user_id, login_id, login_pw, name, created_at)
- [x] sessions relationship 정의

**ChatSession 모델**
```python
class ChatSession(Base):
    """
    채팅 세션
    - 절대 삭제하지 않음
    - is_active / closed_at 으로만 상태 관리
    """
    __tablename__ = "chat_session"

    session_id = Column(String(100), primary_key=True, index=True)
    user_id = Column(
        Integer,
        ForeignKey("user.user_id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    created_at = Column(DateTime, server_default=func.now())
    is_active = Column(Boolean, default=True)
    closed_at = Column(DateTime, nullable=True)

    user = relationship("User", back_populates="sessions")

    chat_histories = relationship(
        "ChatHistory",
        back_populates="session",
        cascade="save-update, merge",
        passive_deletes=True,
    )

    generation_histories = relationship(
        "GenerationHistory",
        back_populates="session",
        cascade="save-update, merge",
        passive_deletes=True,
    )
```

- [x] ChatSession 클래스 작성
- [x] is_active, closed_at 필드 추가
- [x] Relationships 정의

**ChatHistory 모델**
```python
class ChatHistory(Base):
    """
    채팅 메시지 단위 로그
    - 이미지 직접 FK 없음
    """
    __tablename__ = "chat_history"

    id = Column(Integer, primary_key=True, index=True)

    session_id = Column(
        String(100),
        ForeignKey("chat_session.session_id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )

    role = Column(String(20), nullable=False)  # user / assistant
    content = Column(Text, nullable=False)

    created_at = Column(DateTime, server_default=func.now())

    session = relationship("ChatSession", back_populates="chat_histories")

    history_images = relationship(
        "HistoryImage",
        back_populates="chat_history",
        cascade="save-update, merge",
        passive_deletes=True,
    )
```
- [x] ChatHistory 클래스 작성


**GenerationHitory 모델**

```python
class GenerationHistory(Base):
    """
    광고/이미지/텍스트 생성 이벤트 로그
    - 이미지는 HistoryImage 통해 연결
    """
    __tablename__ = "generation_history"

    id = Column(Integer, primary_key=True, index=True)

    session_id = Column(
        String(100),
        ForeignKey("chat_session.session_id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )

    content_type = Column(String(20), nullable=False)  # image / text / both

    input_text = Column(Text)
    output_text = Column(Text)

    generation_method = Column(String(50))
    style = Column(String(50))
    industry = Column(String(50))
    seed = Column(Integer)
    aspect_ratio = Column(String(10))
    created_at = Column(DateTime, server_default=func.now())

    session = relationship("ChatSession", back_populates="generation_histories")

    history_images = relationship(
        "HistoryImage",
        back_populates="generation_history",
        cascade="save-update, merge",
        passive_deletes=True,
    )
```

- [x] GenerationHistory 클래스 작성
- [x] output_text, aspect_ratio 필드 추가
- [x] user_id 제거 (session 통해 간접 참조)


**ImageMatching 모델**
```python
class ImageMatching(Base):
    """
    이미지 파일 레지스트리 (캐시 테이블)
    - file_hash 기준 단일 row
    """
    __tablename__ = "image_matching"

    id = Column(Integer, primary_key=True)
    file_hash = Column(String(128), nullable=False, unique=True, index=True)
    file_directory = Column(Text, nullable=False)
    created_at = Column(DateTime, server_default=func.now())

    history_images = relationship(
        "HistoryImage",
        back_populates="image",
        passive_deletes=True,
    )
```

- [x] ImageMatching 클래스 작성
- [x] file_type 제거 (통합)
- [x] history_images relationship 추가



**HistoryImage 모델** (신규)
```python
class HistoryImage(Base):
    """
    ChatHistory / GenerationHistory 공용 이미지 연결 테이블
    - 이미지 역할(role) 관리
    - 둘 중 하나의 FK만 채워져야 함
    """

    __tablename__ = "history_image"

    id = Column(Integer, primary_key=True)

    chat_history_id = Column(
        Integer,
        ForeignKey("chat_history.id", ondelete="RESTRICT"),
        nullable=True,
        index=True,
    )

    generation_history_id = Column(
        Integer,
        ForeignKey("generation_history.id", ondelete="RESTRICT"),
        nullable=True,
        index=True,
    )

    image_id = Column(
        Integer,
        ForeignKey("image_matching.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )

    role = Column(String(20), nullable=False)  # input / output / reference / mask
    position = Column(Integer, nullable=True)
    created_at = Column(DateTime, server_default=func.now())

    __table_args__ = (
        CheckConstraint(
            "(chat_history_id IS NOT NULL AND generation_history_id IS NULL) OR "
            "(chat_history_id IS NULL AND generation_history_id IS NOT NULL)",
            name="ck_history_image_owner",
        ),
    )

    chat_history = relationship("ChatHistory", back_populates="history_images")
    generation_history = relationship("GenerationHistory", back_populates="history_images")
    image = relationship("ImageMatching", back_populates="history_images")
```

- [x] HistoryImage 클래스 작성 (신규)
- [x] role, position 필드 정의
- [x] 3-way relationship 정의

**모델 검증**
```bash
python -c "from src.backend.models import Base; print('✅ 모델 로드 성공')"
```

- [x] 파일 실행 테스트
- [x] Import 오류 없음 확인

---

#### Day 6-7 (1/3-1/4): process_db.py 및 schemas.py

**process_db.py 기본 구조**

파일: `src/backend/process_db.py`

```python
"""
DB 접근 레이어
SQLAlchemy CRUD 함수
"""

from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker, Session
from typing import Optional, List
from datetime import datetime
import uuid

from . import models, schemas
from utils.config import settings
from utils.auth_utils import hash_password, verify_password

# 엔진 생성
engine = create_engine(
    settings.DATABASE_URL,
    pool_pre_ping=True,
    pool_size=10,
    max_overflow=20
)

# 세션 팩토리
SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)

def init_db():
    """테이블 생성"""
    models.Base.metadata.create_all(bind=engine)
    print("✅ 테이블 생성 완료")

def get_db():
    """FastAPI 의존성 주입"""
    db = SessionLocal()
    try:
        yield db
    finally:
        db.close()
```

- [x] 기본 구조 작성
- [x] 엔진 및 세션 설정
- [x] init_db(), get_db() 함수

**User CRUD 함수**
```python
def get_user_by_login_id(db: Session, login_id: str) -> Optional[models.User]:
    return db.query(models.User).filter(
        models.User.login_id == login_id
    ).first()

def create_user(db: Session, user_data: schemas.UserCreate) -> models.User:
    hashed_pw = hash_password(user_data.login_pw)
    db_user = models.User(
        login_id=user_data.login_id,
        login_pw=hashed_pw,
        name=user_data.name
    )
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user

def authenticate_user(db: Session, login_data: schemas.UserLogin):
    user = get_user_by_login_id(db, login_data.login_id)
    if not user:
        return None
    if not verify_password(login_data.login_pw, user.login_pw):
        return None
    return user
```

- [x] get_user_by_login_id() 작성
- [x] create_user() 작성 (비밀번호 해싱 포함)
- [x] authenticate_user() 작성

**ChatSession CRUD 함수**
```python
def create_session(db: Session, user_id: Optional[int] = None):
    session_id = str(uuid.uuid4())
    db_session = models.ChatSession(
        session_id=session_id,
        user_id=user_id,
        is_active=True
    )
    db.add(db_session)
    db.commit()
    db.refresh(db_session)
    return db_session

def close_session(db: Session, session_id: str):
    session = db.query(models.ChatSession).filter(
        models.ChatSession.session_id == session_id
    ).first()
    if session:
        session.is_active = False
        session.closed_at = datetime.utcnow()
        db.commit()
    return session

def get_active_sessions(db: Session, user_id: int, limit: int = 20):
    return db.query(models.ChatSession).filter(
        models.ChatSession.user_id == user_id,
        models.ChatSession.is_active == True
    ).order_by(
        models.ChatSession.created_at.desc()
    ).limit(limit).all()
```

- [ ] create_session() 작성
- [ ] close_session() 작성 (신규)
- [ ] get_active_sessions() 작성 (신규)

**테이블 생성**
```bash
cd ~/codeit_ad_smallbiz
python -c "from src.backend.process_db import init_db; init_db()"

# 확인
psql -U ad_user -d adbizdb -c "\dt"
```

- [ ] 테이블 생성 실행
- [ ] 6개 테이블 확인

**schemas.py 작성**

파일: `src/backend/schemas.py`

```python
"""
Pydantic 스키마
"""

from pydantic import BaseModel, ConfigDict
from datetime import datetime
from typing import Optional, Literal

# User 스키마
class UserCreate(BaseModel):
    login_id: str
    login_pw: str
    name: str

class UserResponse(BaseModel):
    user_id: int
    login_id: str
    name: str
    created_at: datetime
    
    model_config = ConfigDict(from_attributes=True)

class UserLogin(BaseModel):
    login_id: str
    login_pw: str

# Generation 스키마
class GenerationRequest(BaseModel):
    session_id: str
    input_text: str
    aspect_ratio: Literal["1:1", "4:3", "3:4"]

class GenerationResponse(BaseModel):
    id: int
    session_id: str
    content_type: str
    input_text: Optional[str]
    output_text: Optional[str]
    aspect_ratio: Optional[str]
    created_at: datetime
    
    model_config = ConfigDict(from_attributes=True)
```

- [x] UserCreate, UserResponse, UserLogin 작성
- [ ] GenerationRequest, GenerationResponse 작성
- [ ] aspect_ratio, output_text 필드 포함

---

### 🗓 2주차 (1/5 ~ 1/11): CRUD 함수 완성 및 services.py 시작

#### Day 8-9 (1/5-1/6): 나머지 CRUD 함수

**ChatHistory CRUD**
```python
def save_chat_message(db: Session, session_id: str, role: str, content: str):
    db_message = models.ChatHistory(
        session_id=session_id,
        role=role,
        content=content
    )
    db.add(db_message)
    db.commit()
    db.refresh(db_message)
    return db_message

def get_chat_history(db: Session, session_id: str, limit: Optional[int] = None):
    query = db.query(models.ChatHistory).filter(
        models.ChatHistory.session_id == session_id
    ).order_by(models.ChatHistory.created_at)
    
    if limit:
        query = query.limit(limit)
    
    return query.all()
```

- [ ] save_chat_message() 작성
- [ ] get_chat_history() 작성

**GenerationHistory CRUD**
```python
def save_generation(db: Session, generation_data: dict):
    db_gen = models.GenerationHistory(
        session_id=generation_data["session_id"],
        content_type=generation_data["content_type"],
        input_text=generation_data.get("input_text"),
        output_text=generation_data.get("output_text"),
        generation_method=generation_data.get("generation_method"),
        style=generation_data.get("style"),
        industry=generation_data.get("industry"),
        seed=generation_data.get("seed"),
        aspect_ratio=generation_data.get("aspect_ratio")
    )
    db.add(db_gen)
    db.commit()
    db.refresh(db_gen)
    return db_gen
```

- [ ] save_generation() 작성

**ImageMatching 및 HistoryImage CRUD**
```python
def save_image(db: Session, file_hash: str, file_directory: str):
    # 중복 체크
    existing = db.query(models.ImageMatching).filter(
        models.ImageMatching.file_hash == file_hash
    ).first()
    
    if existing:
        return existing
    
    db_image = models.ImageMatching(
        file_hash=file_hash,
        file_directory=file_directory
    )
    db.add(db_image)
    db.commit()
    db.refresh(db_image)
    return db_image

def save_history_image(db: Session, image_data: dict):
    db_history_image = models.HistoryImage(
        chat_history_id=image_data.get("chat_history_id"),
        generation_history_id=image_data.get("generation_history_id"),
        image_id=image_data["image_id"],
        role=image_data["role"],
        position=image_data["position"]
    )
    db.add(db_history_image)
    db.commit()
    db.refresh(db_history_image)
    return db_history_image
```

- [ ] save_image() 작성 (중복 체크 포함)
- [ ] save_history_image() 작성

---

#### Day 10-11 (1/7-1/8): services.py 기본 구조

**파일 생성**: `src/backend/services.py`

```python
"""
비즈니스 로직
광고 생성 워크플로우
"""

import hashlib
import os
from datetime import datetime
from typing import Dict, Any
import logging

from . import process_db
# 이하 내용은 추후 작성된 내용 확인 필요
from generation.image_generation.generator import SDXLGenerator
from generation.text_generation.prompt_manager import PromptTemplateManager
from utils.config import settings

logger = logging.getLogger(__name__)

# 전역 객체 (한 번만 로드)
sdxl_generator = None
prompt_manager = None

def init_generators():
    """AI 모델 초기화"""
    global sdxl_generator, prompt_manager
    
    try:
        sdxl_generator = SDXLGenerator()
        prompt_manager = PromptTemplateManager()
        logger.info("✅ AI 모델 초기화 완료")
    except Exception as e:
        logger.error(f"❌ AI 모델 초기화 실패: {e}")
        raise
```

- [ ] 파일 생성 및 기본 import
- [ ] init_generators() 함수 작성

**광고 생성 메인 함수 (골격)**
```python
def generate_advertisement(
    db,
    session_id: str,
    user_input: str,
    aspect_ratio: str = "1:1",
    style: str = "realistic",
    industry: str = None
) -> Dict[str, Any]:
    """
    광고 생성 메인 함수
    
    Args:
        db: DB 세션
        session_id: 채팅 세션 ID
        user_input: 사용자 입력 텍스트
        aspect_ratio: 이미지 비율
        style: 스타일
        industry: 업종
    
    Returns:
        생성 결과 딕셔너리
    """
    try:
        logger.info(f"🎨 광고 생성 시작: {user_input[:50]}...")
        
        # 1. 프롬프트 생성 (4번 담당자)
        # 2. 이미지 생성 (3번 담당자)
        # 3. 광고 문구 생성 (4번 담당자)
        # 4. DB 저장
        # 5. 결과 반환
        
        pass  # Day 12-14에서 구현
        
    except Exception as e:
        logger.error(f"❌ 광고 생성 실패: {e}")
        raise
```

- [ ] generate_advertisement() 골격 작성
- [ ] 주석으로 플로우 정리

---

#### Day 12-14 (1/9-1/11): services.py 구현 완성

**1단계: 프롬프트 생성**
```python
def generate_advertisement(db, session_id: str, user_input: str, 
                          aspect_ratio: str = "1:1", style: str = "realistic",
                          industry: str = None) -> Dict[str, Any]:
    try:
        logger.info(f"🎨 광고 생성 시작")
        
        # 1. 프롬프트 생성 (PromptTemplateManager 호출)
        image_prompt = prompt_manager.generate_image_prompt(
            user_input=user_input,
            style=style
        )
        logger.info(f"✅ 프롬프트 생성 완료: {image_prompt[:50]}...")
        
        # ... (계속)
```

- [ ] prompt_manager.generate_image_prompt() 호출
- [ ] 로깅 추가

**2단계: 이미지 생성**
```python
        # 2. 이미지 생성 (SDXLGenerator 호출)
        generation_result = sdxl_generator.generate(
            prompt=image_prompt,
            aspect_ratio=aspect_ratio
        )
        
        image_path = generation_result["image_path"]
        seed = generation_result["seed"]
        
        logger.info(f"✅ 이미지 생성 완료: {image_path}")
```

- [ ] sdxl_generator.generate() 호출
- [ ] 이미지 경로 및 seed 받기

**3단계: 파일 해시 계산**
```python
        # 3. 이미지 파일 해시 계산
        with open(image_path, 'rb') as f:
            file_hash = hashlib.sha256(f.read()).hexdigest()
        
        logger.info(f"✅ 파일 해시: {file_hash[:16]}...")
```

- [ ] SHA-256 해시 계산

**4단계: 광고 문구 생성**
```python
        # 4. 광고 문구 생성 (PromptTemplateManager 호출)
        ad_copy = prompt_manager.generate_ad_copy(
            user_input=user_input,
            style=style,
            industry=industry
        )
        
        logger.info(f"✅ 광고 문구 생성: {ad_copy[:30]}...")
```

- [ ] generate_ad_copy() 호출 (4번 담당자와 협의)

**5단계: DB 저장 (통합 워크플로우)**
```python
        # 5. DB 저장
        # 5-1. 생성 이력
        generation = process_db.save_generation(db, {
            "session_id": session_id,
            "content_type": "image",
            "input_text": user_input,
            "output_text": ad_copy,
            "generation_method": "t2i",
            "style": style,
            "industry": industry,
            "seed": seed,
            "aspect_ratio": aspect_ratio
        })
        
        # 5-2. 이미지 파일
        image = process_db.save_image(db, file_hash, image_path)
        
        # 5-3. 이미지 연결
        process_db.save_history_image(db, {
            "generation_history_id": generation.id,
            "image_id": image.id,
            "role": "output",
            "position": 1
        })
        
        # 5-4. 채팅 이력 (사용자)
        user_msg = process_db.save_chat_message(
            db, session_id, "user", user_input
        )
        
        # 5-5. 채팅 이력 (AI)
        ai_msg = process_db.save_chat_message(
            db, session_id, "assistant", ad_copy
        )
        
        # 5-6. AI 메시지에 이미지 연결
        process_db.save_history_image(db, {
            "chat_history_id": ai_msg.id,
            "image_id": image.id,
            "role": "output",
            "position": 1
        })
        
        logger.info(f"✅ DB 저장 완료")
```

- [ ] 전체 워크플로우 구현
- [ ] 6단계 DB 저장 (generation, image, history_image, chat x2, history_image)

**6단계: 결과 반환**
```python
        # 6. 결과 반환
        return {
            "generation_id": generation.id,
            "image_path": image_path,
            "image_url": f"/api/images/{file_hash}",
            "ad_copy": ad_copy,
            "seed": seed,
            "aspect_ratio": aspect_ratio,
            "style": style
        }
        
    except Exception as e:
        logger.error(f"❌ 광고 생성 실패: {e}")
        db.rollback()
        raise
```

- [ ] 결과 딕셔너리 반환
- [ ] 예외 처리 (rollback 포함)

**전체 함수 테스트**
```python
# 테스트 코드
if __name__ == "__main__":
    from process_db import SessionLocal, init_db, create_session
    
    # 초기화
    init_db()
    init_generators()
    
    # DB 세션
    db = SessionLocal()
    
    try:
        # 세션 생성
        session = create_session(db)
        
        # 광고 생성
        result = generate_advertisement(
            db,
            session_id=session.session_id,
            user_input="카페 신메뉴 홍보, 따뜻한 느낌",
            aspect_ratio="1:1",
            style="realistic",
            industry="cafe"
        )
        
        print("✅ 생성 완료!")
        print(f"이미지: {result['image_path']}")
        print(f"문구: {result['ad_copy']}")
        
    finally:
        db.close()
```

- [ ] 테스트 코드 작성
- [ ] 전체 플로우 테스트

---

### 🗓 3주차 (1/12 ~ 1/15): 통합 및 최적화

#### Day 15-16 (1/12-1/13): 이현석님, 배현석님과 통합

**이현석님과 협업**
- [ ] SDXLGenerator 인터페이스 확인
- [ ] generate() 함수 시그니처 확인
- [ ] 반환 형식 통일 (image_path, seed)
- [ ] 에러 처리 협의

**배현석님과 협업**
- [ ] PromptTemplateManager 인터페이스 확인
- [ ] generate_image_prompt() 함수 확인
- [ ] generate_ad_copy() 함수 협의 (신규)
- [ ] 반환 형식 통일

**통합 테스트**
```python
# 이현석님 코드 import
from generation.image_generation.generator import SDXLGenerator

# 배현석님 코드 import
from generation.text_generation.prompt_manager import PromptTemplateManager

# 통합 테스트
generator = SDXLGenerator()
prompt_mgr = PromptTemplateManager()

# 실제 생성
result = generate_advertisement(db, ...)
```

- [ ] 전체 플로우 통합 테스트
- [ ] 에러 케이스 테스트

---

#### Day 17-18 (1/14-1/15): 이유노님과 API 연동

**이유노님과 협업**

**routes.py에서 services.py 호출**
```python
# routes.py (이유노님 작성)
from fastapi import APIRouter, Depends
from sqlalchemy.orm import Session
from . import services, schemas
from .process_db import get_db

router = APIRouter()

@router.post("/generate")
def generate_ad_api(
    request: schemas.GenerationRequest,
    db: Session = Depends(get_db)
):
    """광고 생성 API"""
    result = services.generate_advertisement(
        db=db,
        session_id=request.session_id,
        user_input=request.input_text,
        aspect_ratio=request.aspect_ratio
    )
    
    return {
        "generation_id": result["generation_id"],
        "image_url": result["image_url"],
        "ad_copy": result["ad_copy"]
    }
```

- [ ] API 엔드포인트 연동
- [ ] 요청/응답 형식 협의
- [ ] 에러 처리 협의

**Postman 테스트**
```json
POST /api/generate
{
    "session_id": "abc-123-def",
    "input_text": "카페 신메뉴 홍보",
    "aspect_ratio": "1:1"
}
```

- [ ] API 테스트 (Postman)
- [ ] 전체 플로우 검증

---

## 📅 2단계 (개선): 2026-01-16 ~ 27

### 🗓 4주차 (1/16 ~ 1/22): 고급 기능

#### Day 19-20 (1/16-1/17): Eager Loading 최적화

**N+1 문제 해결**
```python
from sqlalchemy.orm import joinedload

def get_chat_history_with_images(db: Session, session_id: str):
    return db.query(models.ChatHistory).options(
        joinedload(models.ChatHistory.images).joinedload(
            models.HistoryImage.image
        )
    ).filter(
        models.ChatHistory.session_id == session_id
    ).order_by(
        models.ChatHistory.created_at
    ).all()
```

- [ ] joinedload 적용
- [ ] 쿼리 개수 확인 (before/after)

---

#### Day 21-22 (1/18-1/19): 페이지네이션

**페이지네이션 함수**
```python
def get_paginated_generations(
    db: Session,
    user_id: int,
    page: int = 1,
    page_size: int = 20
):
    from sqlalchemy import func
    import math
    
    total = db.query(func.count(models.GenerationHistory.id)).join(
        models.ChatSession
    ).filter(
        models.ChatSession.user_id == user_id
    ).scalar()
    
    skip = (page - 1) * page_size
    items = db.query(models.GenerationHistory).join(
        models.ChatSession
    ).filter(
        models.ChatSession.user_id == user_id
    ).order_by(
        models.GenerationHistory.created_at.desc()
    ).offset(skip).limit(page_size).all()
    
    return {
        "items": items,
        "total": total,
        "page": page,
        "page_size": page_size,
        "total_pages": math.ceil(total / page_size)
    }
```

- [ ] 페이지네이션 구현
- [ ] API 연동 (이유노님)

---

#### Day 23-24 (1/20-1/21): 비동기 처리 (선택)

**FastAPI 비동기 지원**
```python
# services.py
async def generate_advertisement_async(
    db,
    session_id: str,
    user_input: str,
    ...
):
    """비동기 광고 생성"""
    # AI 생성은 블로킹이므로 ThreadPoolExecutor 사용
    import asyncio
    from concurrent.futures import ThreadPoolExecutor
    
    executor = ThreadPoolExecutor(max_workers=2)
    loop = asyncio.get_event_loop()
    
    # 이미지 생성을 별도 스레드에서
    image_task = loop.run_in_executor(
        executor,
        sdxl_generator.generate,
        image_prompt,
        aspect_ratio
    )
    
    result = await image_task
    
    # ... (나머지 동일)
```

- [ ] 비동기 버전 구현 (선택)
- [ ] 성능 테스트

---

### 🗓 5주차 (1/23 ~ 1/27): 마무리

#### Day 25-26 (1/23-1/24): 통계 및 대시보드 API

**사용자 통계**
```python
def get_user_statistics(db: Session, user_id: int):
    from sqlalchemy import func
    
    stats = {
        "total_sessions": db.query(func.count(models.ChatSession.session_id))
            .filter(models.ChatSession.user_id == user_id).scalar(),
        
        "total_generations": db.query(func.count(models.GenerationHistory.id))
            .join(models.ChatSession)
            .filter(models.ChatSession.user_id == user_id).scalar(),
        
        "total_messages": db.query(func.count(models.ChatHistory.id))
            .join(models.ChatSession)
            .filter(models.ChatSession.user_id == user_id).scalar()
    }
    
    return stats
```

- [ ] 통계 함수 구현
- [ ] API 엔드포인트 연동

---

#### Day 27-30 (1/25-1/27): 문서화 및 최종 테스트

**Docstring 추가**
- [ ] 모든 함수에 설명 추가
- [ ] 타입 힌트 확인

**통합 테스트**
- [ ] 전체 플로우 테스트
- [ ] 에러 케이스 테스트
- [ ] 성능 테스트

**문서 작성**
- [ ] API 문서 (1번 담당자와 협업)
- [ ] DB 스키마 문서
- [ ] 배포 가이드 (5번 담당자와 협업)

---

## 🤝 협업 포인트

### 이유노님 (API)
- [ ] schemas.py 공유
- [ ] get_db() 의존성 사용법
- [ ] services.generate_advertisement() 호출
- [ ] 에러 응답 형식 통일

### 이현석님 (이미지 생성)
- [ ] SDXLGenerator 인터페이스 확인
- [ ] generate() 반환 형식 통일
- [ ] 에러 처리 협의

### 배현석님 (텍스트 생성)
- [ ] PromptTemplateManager 인터페이스 확인
- [ ] generate_ad_copy() 함수 협의
- [ ] 프롬프트 형식 통일

### 신승목님 (인프라)
- [ ] DATABASE_URL 환경 변수
- [ ] 테이블 생성 스크립트
- [ ] 이미지 저장 경로 확인

---

## 📝 체크리스트 요약

### Week 1: DB 설계
- [ ] PostgreSQL 설치 및 설정
- [ ] models.py 작성 (6개 테이블)
- [ ] process_db.py 기본 구조
- [ ] schemas.py 작성
- [ ] 테이블 생성 완료

### Week 2: CRUD 및 services.py
- [ ] 모든 CRUD 함수 작성
- [ ] services.py 기본 구조
- [ ] generate_advertisement() 구현
- [ ] 통합 워크플로우 완성

### Week 3: 통합 및 테스트
- [ ] 3번, 4번 담당자와 통합
- [ ] 1번 담당자와 API 연동
- [ ] 전체 플로우 테스트

### Week 4: 고급 기능
- [ ] Eager Loading 최적화
- [ ] 페이지네이션
- [ ] 통계 API
- [ ] 문서화

---

**작성일**: 2025-01-05  
**버전**: 최종 통합본
**담당자**: 진수경님 (백엔드 비즈니스 로직)  
**최종 수정**: 2026-01-05