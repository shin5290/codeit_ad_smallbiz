...
- [ ] `chatbot.py` 스켈레톤 생성
  ...
  - [ ] get_chatbot() 싱글톤 함수
- [ ] DB 스키마 확장
  - [ ] models.py: GenerationHistory에 컬럼 추가
  - [ ] 마이그레이션 스크립트 작성/실행
- [ ] schemas.py 확장
  - [ ] ReviseRequest 스키마
  - [ ] ConfirmRequest 스키마

### Phase 2: RAG 챗봇 핵심 (3-4일)

- [ ] ConversationManager 구현
  - [ ] add_message() 함수
  - [ ] get_recent_messages() 함수
  - [ ] get_generation_history() 함수
  - [ ] search_similar_messages() 스텁
- [ ] LLMOrchestrator 구현
  - [ ] analyze_intent() 함수 (모든 경우에 사용)
  - [ ] generate_consulting_response() 함수 (상담 intent만)
  - [ ] 프롬프트 템플릿 작성
- [ ] RAGChatbot.process_message() 구현
  - [ ] 의도 분석 통합
  - [ ] VectorDB 검색 통합
  - [ ] Intent별 분기 처리 (생성/수정 vs 상담)
  - [ ] 대화 저장
- [ ] services.py 업데이트
  - [ ] handle_chat_message() 완성 (Intent별 분기 처리)
  - [ ] 생성/수정 intent → 생성 파이프라인 호출
  - [ ] 상담 intent → 상담 응답 반환

### Phase 3: 수정/컨펌 플로우 (2-3일)

- [ ] services.py 보완
  - [ ] handle_chat_revise() 함수
  - [ ] handle_chat_confirm() 함수
- [ ] routers/chat.py 보완
  - [ ] POST /chat/revise 엔드포인트
  - [ ] POST /chat/confirm 엔드포인트
- [ ] process_db.py 보완 (필요시)
  - [ ] revision 관련 쿼리 함수

### Phase 4: 정적 파일 VectorDB (2-3일)

- [ ] consulting_knowledge_base.py 작성
  - [ ] ConsultingKnowledgeBase 인터페이스
  - [ ] MockKnowledgeBase 구현
- [ ] chatbot.py 통합
  - [ ] RAGChatbot에 knowledge_base 파라미터
  - [ ] process_message()에서 지식베이스 검색
- [ ] 팀원 협업
  - [ ] 인터페이스 문서 공유
  - [ ] 실제 구현 대기/통합

### Phase 5: pgvector 통합 (선택, 2-3일)

- [ ] PostgreSQL pgvector 설치
- [ ] DB 스키마 변경 (embedding 컬럼)
- [ ] utils/embedding.py 작성
- [ ] ConversationManager 업데이트
  - [ ] 임베딩 생성 및 저장
  - [ ] search_similar_messages() 구현
