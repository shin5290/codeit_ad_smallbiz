...
**admin_logs.py**
- `get_log_files()`: 로그 파일 목록 조회
- `read_log_file()`: 로그 파일 내용 읽기
- `parse_log_entry()`: 로그 엔트리 파싱

**intent_keywords.py**
- Intent 분류용 키워드 매핑 정의
- 생성/수정/상담 의도 분류 지원
...
- `03_build_documents.py`: LangChain Document 빌드
- `04_build_vectorstore.py`: ChromaDB 벡터스토어 빌드
...
**평가 모듈** (`evaluation/`):
- `01_generate_queries.py`: 평가용 쿼리 생성
- `02_evaluate_recall.py`: Recall@K 평가
- `03_evaluate_hybrid_reranker.py`: 하이브리드 리랭커 평가
- `04_evaluate_advanced_metrics.py`: 고급 메트릭 평가
- `05_evaluate_query_rewriting.py`: 쿼리 재작성 평가
- `06_end_to_end_eval.py`: E2E 평가
...
│   │   │   ├── download_model_zit.py     # Z-Image Turbo 모델 다운로드
│   │   │   ├── nodes/                    # 생성 노드
│   │   │   │   ├── base.py               # 기본 노드
...
│   │       │   ├── 03_build_documents.py # 문서 빌드
│   │       │   └── 04_build_vectorstore.py   # 벡터스토어 빌드
│   │       └── evaluation/               # 평가
