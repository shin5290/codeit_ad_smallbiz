...
**파일**: `src/backend/routers/chat.py`

- `POST /chat/message` 엔드포인트를 `StreamingResponse`로 변경
- `handle_chat_message`에서 상담(intent="consulting") 응답을 스트리밍으로 전송
...
**파일**: `src/backend/task.py`, `src/backend/services.py`

- `TaskState` 클래스에 `status_message: str` 필드 추가
- `update_task_progress` 함수에 `message` 파라미터 추가
- 주요 단계별 메시지 설정:
  - "의도 분석 중..."
  - "이미지 생성 중... (스타일: 실사)"
  - "광고 문구 생성 중..."
...
- `pollTask` 함수에서 `status_message` 읽어서 표시
- 스피너와 함께 진행 상태 메시지 업데이트

**변경 사항**:
```javascript
// pollTask 내부
const message = status.status_message || "처리 중...";
updateAssistantPlaceholder(placeholderId, `${message} (${progress}%)`);
```
...
- 생성 타입 구분: `image`, `text`, `both`
- 플랫폼 감지: `instagram`(1:1), `youtube`(16:9), `story`(9:16), `facebook`(4:3)
...
- `handle_generate_pipeline`에서 플랫폼 기반 비율 자동 적용
- 플랫폼별 기본값 매핑:
  - `instagram`: "1:1"
  - `youtube`: "16:9"
  - `story`: "9:16"
