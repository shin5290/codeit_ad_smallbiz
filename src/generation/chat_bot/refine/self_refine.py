...
2) **근거 (evidence)**
   - 참고 매장이 명시되어 있는가? (매장명, 지역)
   - 제공된 사례 범위 내에서만 근거를 썼는가?
   - 0점: 참고 매장 없음 / 10점: 핵심 조언에 참고 매장 있음
...
- 참고 매장이 없으면 "근거 부족"으로 지적하되, 임의 수치 추가를 권하지 말 것
...
3) 참고 매장이 부족하면 **원문에 있는 매장만 재정리** (새 매장/출처 생성 금지)
4) 결과 보장 표현 제거 ("반드시" → "~할 수 있습니다")
5) 평점/별점 언급 제거
6) 원래 답변의 핵심 내용은 유지
7) 참고 매장이 없으면 섹션을 생략 (억지 표기 금지)
