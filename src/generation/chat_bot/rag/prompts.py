...
2) 근거 표기: 각 주장에 근거를 붙이고, 출처는 {제목}({지역}) 형식
...
9) 숫자/근거: 근거가 있는 항목에만 숫자(빈도/예산/기간 등) 포함. 근거가 없으면 '수치 근거 없음' 명시
10) 출처 없음 처리: 출처를 못 찾으면 "출처: 제공된 자료에서 확인 불가"로 명시
11) 톤: 친근하지만 간결, 불필요한 장식/이모지 최소화

## 출력 형식
- 핵심 요약 → 세부 실행 → 출처 순서로 제시"""
...
3) 출처: 정보의 근거 (웹 검색 결과 / 제공된 자료). 없으면 '출처: 제공된 자료에서 확인 불가'
...
2) 실행 전략 3가지: 각 항목마다 **근거가 있는 경우에만** 숫자(빈도/예산/기간/목표 도달 등) 포함. 근거가 없으면 "수치 근거 없음" 표기
3) 예상 효과: **근거가 있는 경우에만** 보수적 수치 제시, 없으면 "수치 근거 없음" 표기
4) 주의사항 2개
5) 다음 단계 체크리스트 3개
채널 우선순위: 인스타그램/틱톡/네이버 중심으로 제안하고, 페이스북 등 노후 채널은 최신 근거가 있을 때만 제한적으로 언급
섹션 제목을 그대로 사용하세요: "현재 상황", "실행", "예상 효과", "주의사항", "다음 단계", "출처".
실행 섹션의 각 항목 끝에 근거를 괄호로 붙이세요. 예) 주 3회, 5만 원, 2주간 (출처: {제목}({지역})). 근거 없으면 "수치 근거 없음" 표기
출처: 하단 출처 섹션에 bullet로 {제목}({지역}) 최소 3개 이상, 중복 없이 명시
제한: 평점/별점 언급 없이 작성""",

    # 트렌드 분석 (Agent 연동용)
    "trend": """최신 트렌드 요약 후 적용 포인트를 제안하세요:
1) 트렌드 요약 3줄
2) 우리 업종 적용 아이디어 3개: **근거가 있는 경우에만** 숫자(빈도/예산/기간 등) 포함하고, 끝에 근거를 괄호로 표기 (출처: {제목}({지역})). 근거 없으면 "수치 근거 없음" 표기
3) 리스크/주의 2개
4) 출처(날짜/링크/사례명) 3개 이상, bullet로 명시""",
