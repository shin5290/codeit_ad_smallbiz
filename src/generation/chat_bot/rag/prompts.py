...
9) 숫자/근거 필수: 각 주요 항목마다 빈도·예산·기간 등 숫자를 2개 이상 포함하고, 근거 사례를 함께 표기(없으면 '수치 없음' 명시)
...
2) 실행 전략 3가지: 각 항목마다 숫자 2개 이상(빈도/예산/기간/목표 도달 등)과 근거 사례를 괄호로 붙이기. 예) 주 3회, 5만 원, 2주간 / 근거: {제목}({지역})
3) 예상 효과: 보수적 수치 2개 이상(예: 도달수, 방문수, 전환률) + 근거 사례 연결
4) 주의사항 2개
5) 다음 단계 체크리스트 3개
채널 우선순위: 인스타그램/틱톡/네이버 중심으로 제안하고, 페이스북 등 노후 채널은 최신 근거가 있을 때만 제한적으로 언급
섹션 제목을 그대로 사용하세요: "현재 상황", "실행", "예상 효과", "주의사항", "다음 단계", "출처".
실행 섹션의 각 항목 끝에 근거를 괄호로 붙이세요. 예) 주 3회, 5만 원, 2주간 (출처: {제목}({지역}))
...
1) 트렌드 요약 3줄
2) 우리 업종 적용 아이디어 3개: 각 아이디어에 숫자(빈도/예산/기간 등) 2개 이상을 넣고, 끝에 근거를 괄호로 표기 (출처: {제목}({지역}))
3) 리스크/주의 2개
